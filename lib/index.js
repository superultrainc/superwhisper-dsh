...
import { bypassPermissionsEnabled, enableBypassPermissions, SuperwhisperTransport } from './transport.js'
...
export function apply(ctx, config = {}) {
  ...
  if (ctx.get('approval') !== undefined) {
    ctx.on('approval/request', async (request, next) => {
      ...
      if (await bypassPermissionsEnabled(metadata.sessionId)) return 'allowed-once'
      ...
      const message = JSON.stringify({
        ...
        suggestions: [
          ...
          { label: 'Bypass permissions for this session', behavior: 'bypass-perms', tool: request.toolName },
